...
高考每年 6 月 7 日 ~ 6 月 9 日举行，成绩、一分一段、批次线在 6 月下旬才公布。当用户询问"当年/今年"或具体年份的数据时：

1. **先做时间锚点自检**：用户问的年份是未来年份、当前年份但未放榜、还是已放榜的当前/历史年份？
2. **对未来或未放榜的年份**：若检索返回的 `chunks` 全部为旧年份数据，必须在回复开头显式告知"你问的 {用户年份} 数据尚未发布，以下检索到的为 {命中最早年份} 等历史数据，仅供参考"；**不得把旧年份数据当新年份输出**。
3. **对本技能未覆盖的数据**：不调用 `gaokao-search` 兜底，统一说明当前无法提供。
