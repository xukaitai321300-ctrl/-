...
description: "Use this skill when the user needs to collect reference images for ComfyUI AI image generation, including product photos, lifestyle scenes, material textures, and style references. Triggers on: 采集图片, 参考图, 素材收集, image collection, reference image, material search. v5.0: Added image search techniques (2026 version), batch download script (Python). Optimized image feature extraction method (increased AI model recommendation). Strengthened bidirectional linkage with tiger expert."
author: "甄宇航（猴哥）"
version: "v5.0"
---

# Image Collection — 虎 (Tiger) v3.5
...
```
**Collection Purpose**:
- [ ] Reference for img2img (HIGH-QUALITY, no defects)
- [ ] Reference for ControlNet (edge/depth/pose)
- [ ] Style reference (color palette, lighting, composition)
- [ ] Material texture (for LoRA training)
- [ ] **Defect samples (for training data) (NEW in v3.5)**
**Product Type**:
- [ ] Vacuum cup / thermos
- [ ] Food jar / lunch box
...
**Image Type Needed**:
- [ ] Product only (pure background)
- [ ] Lifestyle (human using product)
- [ ] Detail close-up (lid mechanism, hinge, seam)
- [ ] **Defect close-up (plastic texture, asymmetric lid, color inaccurate) (NEW in v3.5)**
**Defect Sampling Requirements (NEW in v3.5)**:
- [ ] Collect AI-generated images WITH defects (for training 鸡/兔 to detect)
- [ ] Collect real product images WITH defects (for ComfyUI inpainting fix)
- [ ] Categorize by defect type (structural/material/lighting/text/background/AI)
```
...
**Collection Method**:
```
1. Search keyword: "保温杯 弹跳盖 产品图" (vacuum cup pop-up lid product image)
2. Filter: HIGH-QUALITY, pure background
3. Download: main image + detail images (至少5张)
4. **Also download 差评/negative review images (defect samples) (NEW in v3.5)**
```

### Source 2: ComfyUI Outputs (for AI defect samples)

```
1. Generate images with INTENTIONAL defects (for training):
   - Prompt: "plastic texture, deformed lid, color inaccurate"
   - Collect: images with visible defects
2. Use 兔's defect detection to verify defects are detectable
...
```
**Structural Defects** (collect 10+ samples each):
- [ ] Asymmetric lid (left/right don't match)
- [ ] Deformed handle (bent, twisted)
- [ ] Warped body (not cylindrical)
- [ ] Missing details (hinge not visible, seam not visible)
    # ... (代码已精简，保留核心逻辑) ...
**AI Artifacts** (collect 10+ samples each):
- [ ] High-frequency noise (grainy look)
- [ ] Color noise (random color spots)
- [ ] JPEG compression artifacts (blocky edges)
- [ ] Halo artifacts (around product edges)
```
...
```
## Image Pre-Processing Checklist

**Resize**:
- [ ] Product images: 1024×1024 (square, for training)
- [ ] Lifestyle images: maintain aspect ratio, max 1920px
- [ ] Detail close-ups: 512×512 (focus on detail)

**Rename**:
- [ ] Format: `[product]_[color]_[angle]_[seed].png`
- [ ] Example: `vacuum_cup_yellow_front_123456.png`
- [ ] **Defect samples**: `[defect_type]_[severity]_[source].png` (NEW in v3.5)**

**Metadata** (embed in PNG):
- [ ] Prompt used (if AI-generated)
- [ ] Seed (if AI-generated)
- [ ] Collection source (Taobao/Amazon/ComfyUI)
- [ ] **Defect type + severity (if defect sample) (NEW in v3.5)**
```
...
```
## Defect Pre-Filter Checklist (using 兔's defect detection)

**For HIGH-QUALITY references** (should have NO defects):
- [ ] Run 兔's defect detection on collected images
- [ ] If defect detected → DISCARD (or move to defect samples)
- [ ] Only keep images with NO defects (score ≥ 9.0/10)

**For DEFECT samples** (should have visible defects):
- [ ] Run 兔's defect detection on collected images
- [ ] If NO defect detected → DISCARD (not useful for training)
- [ ] Only keep images WITH defects (score ≤ 6.0/10)

**Categorization**:
- [ ] Sort defect samples by defect type (structural/material/lighting/text/background/AI)
- [ ] Save to: `E:/AI日记/Claw/defect_samples/[defect_type]/`
```
...
- Total collected: [X] images
- HIGH-QUALITY references: [Y] images (no defects)
    # ... (代码已精简，保留核心逻辑) ...
| AI artifacts (noise) | 20 | 3~5 | `defect_samples/ai/` |

## 4. Next Action
- HIGH-quality references → Send to 马 (Horse) for workflow
- Defect samples → Send to 鸡 (Rooster) + 兔 (Rabbit) for training
```
...
|---------|-------|----------|
| Collected images too low quality | Wrong platform/source | Switch to Tmall/Amazon (HIGH-quality product images) |
| Collected images have defects | Didn't pre-filter | Use 兔's defect detection to pre-filter (NEW in v3.5) |
| Defect samples not detectable | Defects too subtle | Collect MORE obvious defect samples (increase severity) (NEW in v3.5) |
    # ... (代码已精简，保留核心逻辑) ...
...
```markdown
### 1. Search Platforms and Techniques:
| Platform | Best For | Search Tips |
|----------|----------|--------------|
| Taobao/Tmall | Chinese market products | Use Pinyin + English keywords |
| Amazon | International products | Use specific model names |
| 1688 | Factory-grade products | Search by material + process |
| Pinterest | Style inspiration | Use English aesthetic keywords |
| Xiaohongshu | Real user photos | Search by use case + scenario |

### 2. Batch Download Script (Python):
```python
import requests
...
- [ ] AI model feature extraction (CLIP, ResNet)
```
