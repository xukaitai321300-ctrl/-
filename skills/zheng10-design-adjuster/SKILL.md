...
description: "Use this skill when the user needs to adjust product design based on review feedback, optimizing appearance details, structural parameters, or CMF matching. Triggers on: 设计调整, 方案优化, 细节改进, design adjustment, optimization, detail refinement, parameter tuning. v5.0: Added parameter tuning methodology complete version, sampler configuration guide (2026 version). Optimized parameter combination case library (increased automated tuning script). Strengthened bidirectional linkage with monkey expert."
author: "甄宇航（猴哥）"
version: "v5.0"
---

# Design Adjustment & Parameter Tuning — 猴 (Monkey) v3.5

**Role**: Design adjuster and parameter tuner. Precisely adjust design based on review feedback, optimize ComfyUI generation parameters.

**Core Principle (v3.5)**: Data-driven adjustment. **Defect-specific tuning** — each type of defect has a specific parameter fix.

---

## Phase 1: Review Feedback Parsing (Enhanced v3.5)

When receiving review report from 鸡 (Rooster) or 兔 (Rabbit):

```
**Review Source**:
- [ ] 鸡 (Rooster) — design review (aesthetics, safety, innovation)
- [ ] 牛 (Ox) — DFM review (manufacturability, cost)
- [ ] 兔 (Rabbit) — image analysis (clarity, realism, artifacts)
**Review Score**:
    # ... (代码已精简，保留核心逻辑) ...
- **Material defects**: plastic texture (should be metal), brushed texture inconsistent
- **Lighting defects**: reflection wrong, shadow direction inconsistent, Fresnel effect missing
- **Text/logo defects**: garbled text, logo deformed, watermark/signature
- **Background defects**: background not pure white, product floating (no contact shadow)
- **AI artifacts**: high-frequency noise, color noise, JPEG compression artifacts
```

---

## Phase 1.5: Defect-to-Parameter Mapping (NEW in v3.5)

**When 鸡/兔 reports a specific defect, use this mapping to tune parameters**:

要点：
- | Defect Type | Root Cause | Parameter Fix | Prompt Fix |
- |-------------|-------------|-----------------|-------------|
- | **Cup body not cylindrical** | Denoise too high (Img2Img) / ControlNet strength too low | **INCREASE** ControlNet strength to 0.9~1.0, **DECREASE** denoise to 0.45~0.50 | Add "perfect cylinder, straight lines, precise geometry" |
- | **Lid asymmetric** | ControlNet Canny not applied / Seed random | **INCREASE** ControlNet strength to 1.0, **FIX** seed value | Add "perfectly symmetric lid, centered pop-up button" |
- | **Handle deformed** | LoRA weight too low / Negative prompt missing | **INCREASE** LoRA weight to 0.8~1.0, **ADD** "deformed handle" to negative | Add "ergonomic handle, precise mechanical form" |
- | **Plastic texture** | Missing "anisotropic reflection" in prompt / LoRA not loaded | **ADD** "anisotropic reflection, brushed metal texture" to prompt, **LOAD** metal-texture LoRA (weight: 0.3~0.5) | Remove "plastic, cheap appearance" from positive, add to negative |
- | **Brushed texture inconsistent** | LoRA trigger word missing / Weight too low | **INCREASE** brushed-titanium LoRA weight to 0.6~0.9, **ADD** trigger word "brushed metal, anisotropic reflection" | Add "consistent brushed texture, directional grain" |
- | **Reflection wrong** | Lighting prompt missing / PBR not enabled | **ADD** "physical based rendering, ray tracing, anisotropic reflection" to prompt | Add "Fresnel effect, physically correct reflection" |
- | **Shadow direction inconsistent** | Lighting prompt conflicting / Background mismatch | **FIX** lighting prompt (e.g., "soft daylight from upper left"), **MATCH** background lighting | Add "consistent shadow direction, physically correct shadow" |
- | **Color inaccurate (ΔE > 5)** | Color keyword missing / CFG too low | **INCREASE** CFG to 8.0~9.0, **ADD** specific color keyword (e.g., "Pantone 1234 C") | Add "color accurate, Delta E < 3, real photography colors" |
- | **Text garbled** | Negative prompt missing "text" / LoRA conflict | **ADD** "text, watermark, signature, garbled text" to negative, **DECREASE** text-related LoRA weight | Add "no text, clean surface, minimalist design" to positive |
- | **Logo deformed** | ControlNet strength too low / Denoise too high | **INCREASE** ControlNet strength to 0.9~1.0, **DECREASE** denoise to 0.50 | Add "precise logo placement, correct perspective" |
- | **Background not pure white** | Background prompt conflicting / CFG too low | **SET** background prompt to "pure white background, studio lighting", **INCREASE** CFG to 8.0 | Add "pure white background, no objects, clean backdrop" |
- | **Product floating (no shadow)** | Missing shadow prompt / Lighting prompt incomplete | **ADD** "contact shadow, soft shadow beneath product, physically correct shadow" to prompt | Add "product resting on surface, stable placement" |
- | **AI noise artifacts** | Steps too few / Sampler too fast | **INCREASE** steps to 40~50, **SWITCH** to DPM++ 2M Karras (not Lightning) | Add "clean render, no noise, no compression artifacts" to negative |
- | **JPEG compression artifacts** | Output format wrong / Quality too low | **CHANGE** output format to PNG (lossless), **INCREASE** resolution to 1024×1024 | Add "no JPEG artifacts, no compression, lossless quality" to negative |
- ---

## Phase 2: Design Adjustment (if design proposal rejected)

If 鸡's review score < 7.0 (design proposal needs adjustment):

```
**Previous Design**: Proposal [A/B/C]
**Review Score**: [X.X]/10
**Main Problems**: [brief description]
| Dimension | Previous | Adjusted | Reason |
|-----------|----------|----------|--------|
| Cup shape | [previous] | [adjusted] | [reason from review] |
| Lid type | [previous] | [adjusted] | [reason from review] |
| Color | [previous] | [adjusted] | [reason from review] |
| Material | [previous] | [adjusted] | [reason from review] |
(Same structure as Phase 2 in 蛇's SKILL.md)
**Previous prompt**:
```
[previous positive prompt]
```

**Adjusted prompt** (v3.5 — ADD defect prevention keywords):
```
[adjusted positive prompt — added defect-specific keywords based on Phase 1.5 mapping]
```

**Negative prompt adjustment**:
```
[adjusted negative prompt — added keywords for defects mentioned in review]
```
```

---

## Phase 3: Parameter Tuning (if AI-generated images need optimization)

If 羊's generated images scored < 7.0 by 鸡:

| Parameter | Previous Value | Adjusted Value | Reason (v3.5 — use Phase 1.5 mapping) |
|-----------|-----------------|------------------|------------------------------------------------|
| Model | [previous] | [adjusted] | [reason, e.g., "Realistic Vision better for product photography"] |
| Sampler | [previous] | [adjusted] | [reason, e.g., "DPM++ 2M Karras sharper"] |
| Steps | [previous] | [adjusted] | [reason, e.g., "increase to 50 for more detail"] |
| CFG Scale | [previous] | [adjusted] | [reason, e.g., "increase to 8.0 for better prompt adherence"] |
| Resolution | [previous] | [adjusted] | [reason, e.g., "increase to 1024×1024 for more detail"] |
| Denoise (Img2Img) | [previous] | [adjusted] | [reason, e.g., "lower to 0.5 to preserve reference structure"] |
| ControlNet strength | [previous] | [adjusted] | [reason, e.g., "increase to 0.9 for stricter edge control"] |
| LoRA weight | [previous] | [adjusted] | [reason, e.g., "increase to 1.0 for stronger style"] |

### Parameter Tuning Lookup Table (v3.5 Enhanced):

要点：
- | Problem | Sampler | Steps | CFG | Resolution | Fix (v3.5 — defect-specific) |
- |---------|----------|-------|-----|------------|------------------------------------------|
...
- | Deformed structure | (any) | 30 | 7.0 | (any) | **Lower denoise (Img2Img) OR increase ControlNet strength to 0.9~1.0** |
- | Slow generation | Lightning/Flash | 4~8 | 2.0 | 512×512 | Use accelerated models |
- | **Cup body not cylindrical** | (any) | 30 | 7.0 | (any) | **Increase ControlNet strength to 1.0, lower denoise to 0.45** |
- | **Lid asymmetric** | (any) | 30 | 7.0 | (any) | **Increase ControlNet strength to 1.0, FIX seed** |
- | **Plastic texture** | DPM++ 2M Karras | 40 | 8.0 | 1024×1024 | **Add "anisotropic reflection" to prompt, ADD metal-texture LoRA** |
- | **Color inaccurate (ΔE > 5)** | (any) | 30 | 8.0~9.0 | (any) | **Increase CFG to 8.0~9.0, ADD specific color keyword** |
- | **Text garbled** | (any) | 30 | 7.0 | (any) | **Add "text, watermark, garbled text" to negative prompt** |
- | **Background not pure white** | (any) | 30 | 8.0 | (any) | **Set background prompt to "pure white background", increase CFG** |
- | **AI noise artifacts** | DPM++ 2M Karras | 40~50 | 7.0 | 1024×1024 | **Increase steps, switch from Lightning to standard sampler** |
- ---

## Phase 3.5: Iterative Defect Fixing (NEW in v3.5)

**When 鸡/兔 reports defects, follow this iterative process**:

```
**Iteration 1**:
- Input: Review report from 鸡/兔
- Action: Parse defects (Phase 1), map to parameters (Phase 1.5)
- Output: Adjusted parameters + prompts
- Expected score improvement: +0.5~1.0
**Iteration 2** (if score still < 7.0):
- Input: Re-review report from 鸡/兔
- Action: Check which defects NOT fixed, adjust parameters AGAIN
- Output: Re-adjusted parameters + prompts
- Expected score improvement: +0.3~0.8
**Iteration 3** (if score still < 7.0):
- Input: Second re-review report
- Action: Major adjustment OR escalate to 鼠 (Rat)
- Output: Major adjustment OR escalation request
- Expected score improvement: +0.2~0.5 OR escalate
**Escalation Condition**:
- If Iteration 3 still < 7.0 → Escalate to 鼠 (Rat) for redesign decision
- 鼠 will: decide to redesign (back to 蛇) OR accept current design (score 6.5~6.9)
```

---

## Phase 4: Design Verification (after adjustment)

After adjusting design or parameters:

```
**DFM Check** (if design adjusted):
- [ ] Draft angle ≥1.5° (for die-casting)
- [ ] Wall thickness ≥0.6mm (for stainless steel)
- [ ] Undercut count = 0 (ideal) or ≤2 (acceptable)
- [ ] Deep slot aspect ratio ≤5:1 (for machining)
**Cost Estimation** (if design adjusted):
- Material cost: ¥[X]/unit
- Processing cost: ¥[X]/unit
- Assembly cost: ¥[X]/unit
- **Total factory price**: ¥[X]/unit (must be ≤¥80 for ¥129 retail)
**Manufacturability Verification** (if design adjusted):
- [ ] Process feasible in Yongkang (local supply)
- [ ] Yield prediction ≥70% (ideally ≥85%)
- [ ] Mold cost acceptable (<¥10K for spinning die)
**Parameter Effectiveness** (if parameters adjusted):
- [ ] Previous score: [X.X]/10
- [ ] Expected score after adjustment: [Y.Y]/10
- [ ] Key parameter changed: [parameter name] → [previous] to [adjusted]
- [ ] **Defect fix rate**: [X] defects fixed out of [Y] total ([X]/[Y] = [Z]%) — target ≥80%
```

---

## Phase 5: Iteration Record & Re-Review Submission

After adjustment, output iteration record and submit for re-review:

```markdown
# Design Adjustment Report (Iteration N)

## 1. Review Feedback Parsed (v3.5 Enhanced)
| Priority | Defect Type | Feedback Content | Dimension | Score Impact | Parameter Fix |
|----------|-------------|-------------------|-----------|-----------------|----------------|
    # ... (代码已精简，保留核心逻辑) ...
## 4. Re-Review Submission
Submit adjusted design to 鸡 (Rooster) for re-review.

**Expected re-review score**: [Y.Y]/10 (must be ≥7.0)
**If still <7.0**: Continue to Iteration N+1 (maximum 3 iterations)
```
| Condition | Action |
|-----------|--------|
| Review score ≥ 8.0/10.0 | ✅ Pass, proceed to next phase (to 羊 for generation) |
| Review score 7.0~7.9/10.0 | ⚠️ Conditional pass, fine-tune parameters (optional) |
| Review score < 7.0/10.0 after adjustment | 🔄 Continue iteration (maximum 3 iterations) |
| Iteration rounds ≥ 3 AND review score still < 7.0 | ❌ Fail, escalate to 鼠 (Rat) for redesign decision |
- Receive: design review report (score < 7.0) OR image review report (score < 7.0)
- Extract: low-scoring dimensions, specific suggestions, **defect types** (NEW in v3.5)
- Output: design adjustment report OR parameter tuning report (v3.5 — use Phase 1.5 mapping)
- Receive: image analysis report (clarity/realism/artifact scores low)
  ... (省略中间部分) ...
...
```
| Problem | Cause | Solution (v3.5 — defect-specific) |
|---------|-------|------------------------------------------------|
| Score improvement too slow (+<0.3 per iteration) | Not fixing root cause | Re-analyze review feedback, identify root cause (not just symptom) |
| P0 feedback not fixed | Missing P0 item | Re-parse review feedback, ensure ALL P0 items fixed |
| DFM score still low after adjustment | Design still too complex | Simplify design further, eliminate undercuts, increase draft angle |
    # ... (代码已精简，保留核心逻辑) ...
...
## Phase 1.2: Parameter Tuning Methodology (NEW in v5.0)

**Complete parameter tuning methodology (2026 version)**:

```markdown
### 1. Parameter Tuning Framework:
| Parameter | Range | Effect | Defect Prevention |
|-----------|--------|--------|-------------------|
| CFG Scale | 5.0-9.0 | Controls prompt adherence | Higher CFG = more accurate color |
| Denoise | 0.3-0.7 | Controls img2img strength | Lower denoise = preserves geometry |
| Steps | 15-30 | Controls generation quality | More steps = less artifacts |
| Sampler | DPM++ 2M Karras | Controls sampling method | DPM++ = best balance |

### 2. Sampler Configuration Guide (2026 version):
- **DPM++ 2M Karras**: Best overall (speed + quality)
- **Euler a**: Fast but less consistent
- **DDIM**: Good for inpainting
- **UniPC**: New sampler, worth testing

### 3. Parameter Combination Case Library:
| Case ID | Defect Type | Parameter Fix | Prompt Fix | Result |
|---------|--------------|----------------|------------|--------|
| PC-001 | Cup body not cylindrical | ControlNet strength 0.9-1.0 | "perfect cylinder" | ✅ Fixed |
| PC-002 | Lid asymmetric | Seed fixed, ControlNet 1.0 | "perfectly symmetric" | ✅ Fixed |
```

**Output**: Parameter Tuning Report (Markdown format)
